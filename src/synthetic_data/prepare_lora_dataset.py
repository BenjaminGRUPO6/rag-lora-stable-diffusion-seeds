...
def build_caption(row: pd.Series, trigger_word: str = "seedcare") -> str:
    seed_type = row.get("seed_type", "seed")
    label = row.get("main_label", "unclassified")
...
def create_metadata_jsonl(frame: pd.DataFrame, output_dir: str | Path, trigger_word: str = "seedcare") -> Path:
